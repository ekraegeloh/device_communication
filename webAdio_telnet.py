...
class webadio:
	...
	global prompt
	global adio_ack
	adio_ack = "\x1b[34done \x1b[0;1m"
	term_char = "\r\n"
	prompt = term_char + ">"

	def login(self, ip, pw):
		try:
			tn = telnetlib.Telnet(ip_adress)
		except Exception, e:
			print "Connection to WebADIO not possible, reason: " + e
			time.sleep(2)
			return False
		...
		try:
			...
			print "Connection not possible, reason: " + e
			time.sleep(2)
			return False
	...
	def read_all(self):
		ai_dict = adio.read_ai()
		ao_dict = adio.read_ao()
		di_dict = adio.read_di()
		do_dict = adio.read_do()
...
def hv_state(hvsupply, state):
	...
	"""
	do_ch = str(descriptor_ao.index("hv_enable_" + str(hvsupply)))
	if state == "1":
...
#make sure all digital outs are activated, eg. in low state
	for i in range(len(descriptor_do)):	adio.activate_do(str(i))


def filter_db_values():
	final_dict = {}
