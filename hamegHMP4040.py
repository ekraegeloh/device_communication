...
class hameg_supply:
	...
	def open_socket(self, ip, port):
		try:
			...
			print 'Connection to HamegHMP4040 not possible, reason: ' + e
			time.sleep(2)
			return False
		hameg_tcp.send("*IDN?\n")
		time.sleep(0.5)
		print "Connected to " + hameg_tcp.recv(buf)
		hameg_tcp.send("*RST\n")
		time.sleep(0.5)
		print "Device reset."
    ...
    def read_volt(self, channel):
        ...
        return string.rstrip(hameg_tcp.recv(buf))
    ...
    def set_curr(self, channel, curr):
        ...
        hameg_tcp.send("*WAI\n")
        hameg_tcp.send("CURR?\n")
        response = float(string.rstrip(hameg_tcp.recv(buf)))
        if response != curr:
